  ...
  const ScoreControl = ({ name, score, player, isWinner }: {
    ...
    >
      <p className={`text-sm font-semibold mb-3 truncate
        ${isWinner ? "text-green-700" : "text-text-secondary"}`}>
        {name}
      </p>
      ...
      {isWinner && (
        <p className="text-xs text-green-600 font-bold mt-2">WINNER</p>
      )}
    </div>
  );
      ...
      >
        ...
        >
          ...
          <div className="px-5 pb-5 space-y-2">
            ...
            >
              {saving ? "Saving..." : `Confirm — ${winnerId === match.player1_id ? p1Name : p2Name} wins`}
            </button>
